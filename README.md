...
- **AI (API) 批量视频分类**：先在可折叠的“设置”中填写并测试 API URL、接口格式、模型和 API Key，再设置每批数量后直接运行。接口格式支持 Chat Completions (`/v1/chat/completions`) 和 Responses (`/v1/responses`)；分类目录生成共用这组配置，配置保存在扩展本地存储中。
